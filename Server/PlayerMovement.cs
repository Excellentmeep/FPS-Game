...
public class PlayerMovement : MonoBehaviour
{
	...
	// Assign references & variables when there's a change made in the editor
	...
	public StatePayload ProcessMovement(InputPayload inputPayload)
	{
		...
		Vector2 velocity = FindVelRelativeToLook(inputPayload.camRotation);
		...
		return new StatePayload()
		{
			...
			camPosition = camProxy.position,
			camRotation = camProxy.forward,
		};
	}

	// Move the player in the correct direction or slide them if their sliding, then send the movement to the clients
	...
	private void Jump()
	{
		...
		if(collisionCheck.Grounded && readyToJump || collisionCheck.OnWall && readyToJump && wallJumpCounter < 5)
		{
			...
			{
				rb.AddForce(camProxy.transform.right * 600);
			...
			if(collisionCheck.OnWall && !collisionCheck.OnWallRight && !collisionCheck.OnWallLeft && !collisionCheck.Grounded)
			{
				rb.AddForce(FlattenVector3(camProxy.forward) * 600);
				rb.AddForce(Vector2.up * jumpForce);
				wallJumpCounter++;
			}
			...
			// Reset the jump cooldown depeding on what jump the player did
	...
	private void StartSlide()
	{
		sliding = true;
		groundedCounter = 0;
		slideDirection = FlattenVector3(camProxy.forward) * Time.fixedDeltaTime;
	...
	// Change different things with speed based depending on different variables
	...
	private Vector2 FindVelRelativeToLook(Vector3 forward)
	{
		this.camProxy.forward = forward;

		float lookAngle = camProxy.transform.eulerAngles.y;
	...
	private void CounterMovement(float x, float y, Vector2 velocity)
	{
		...
		if(Mathf.Abs(velocity.x) > threshold && Mathf.Abs(x) < 0.05f || (velocity.x < -threshold && x > 0) || (velocity.x > threshold && x < 0))
			rb.AddForce(moveSpeed * FlattenVector3(camProxy.transform.right) * Time.fixedDeltaTime * -velocity.x * counterMovement);

		if(Mathf.Abs(velocity.y) > threshold && Mathf.Abs(y) < 0.05f || (velocity.y < -threshold && y > 0) || (velocity.y > threshold && y < 0))
			rb.AddForce(moveSpeed * FlattenVector3(camProxy.transform.forward) * Time.fixedDeltaTime * -velocity.y * counterMovement);
	}
}
